...
def generar_pdf_reporte(
    ...
    estilo_titulo = ParagraphStyle(
        'RepTitle', parent=styles['Heading1'], fontSize=22, leading=26,
        textColor=COLOR_PRIMARIO, spaceAfter=15, keepWithNext=True
    )
    estilo_cuerpo = ParagraphStyle(
        'RepBody', parent=styles['BodyText'], fontSize=11, leading=16,
        textColor=COLOR_TEXTO, spaceAfter=10
    )

    # Encabezado Principal del Reporte
    nombre_reporte = "ANÁLISIS DE RETENCIÓN (CHURN)" if tipo_reporte == "churn" else "RENDIMIENTO FINANCIERO"
    ...
    for linea in lineas:
        ...
        if linea_limpia.startswith('###'):
            estilo_h3 = ParagraphStyle('H3', parent=styles['Heading3'], fontSize=13, leading=16, textColor=COLOR_SECUNDARIO, spaceBefore=8, spaceAfter=4, keepWithNext=True)
            texto_fmt = _convertir_markdown_a_tags_reportlab(linea_limpia.replace('###', '').strip())
            story.append(Paragraph(texto_fmt, estilo_h3))
        elif linea_limpia.startswith('##'):
            estilo_h2 = ParagraphStyle('H2', parent=styles['Heading2'], fontSize=15, leading=18, textColor=COLOR_PRIMARIO, spaceBefore=12, spaceAfter=6, keepWithNext=True)
            texto_fmt = _convertir_markdown_a_tags_reportlab(linea_limpia.replace('##', '').strip())
    ...
    if grafico_base64:
        try:
            ...
            story.append(Spacer(1, 10))
            story.append(Paragraph(f"<i>(Nota: El gráfico adjunto no pudo ser procesado en el PDF: {str(e)})</i>", estilo_cuerpo))
...
async def enviar_por_gmail(buffer_pdf: bytes, destino: str, filename: str = "reporte_violtech.pdf") -> str:
...
async def procesar_confirmacion_envio(canal: str, destino: str = None, contexto_sesion: dict = None) -> tuple:
    """
    ...
    if not texto_reporte:
        return "❌ Error: No encontré el contenido del reporte en el contexto de sesión. Por favor, genéralo de nuevo.", nuevos_estados
    ...
    try:
        buffer_pdf = generar_pdf_reporte(texto_reporte, tipo_reporte, grafico_base64=grafico_b64)
    except Exception as e:
    ...
    try:
        if canal == "gmail":
            if not destino:
                return "❌ Error: Se requiere un correo de destino válido para realizar el envío por Gmail.", nuevos_estados
        ...
        if "Error" in resultado or resultado != "Exitoso":
            return f"❌ Hubo un inconveniente con los servidores de envío: {resultado}", nuevos_estados
        ...
        return f"✅ ¡El reporte en PDF ha sido generado y enviado exitosamente vía {canal.capitalize()}!", nuevos_estados
